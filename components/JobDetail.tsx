...
import { ArrowLeft, MapPin, FileText, ClipboardList, Maximize, Bot, ArrowRight, ToggleLeft, ToggleRight } from 'lucide-react';
...
export const JobDetail: React.FC<JobDetailProps> = ({ job, onStart, onBack }) => {
  const { aiEnabled, toggleAI } = useJobs();
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pb-32 min-h-0">
        ...
        <div className="mb-6 bg-purple-50 p-4 rounded-2xl border border-purple-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h4 className="font-bold text-gray-800 text-sm">Asistente IA</h4>
                    <p className="text-xs text-gray-500">Validación en tiempo real</p>
                </div>
            </div>
            <button 
                onClick={toggleAI}
            ...
            >
                <span className="text-xs font-bold uppercase">{aiEnabled ? 'ON' : 'OFF'}</span>
                <div className={`w-8 h-4 rounded-full bg-black/20 relative`}>
                    <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${aiEnabled ? 'left-4.5' : 'left-0.5'}`} style={{left: aiEnabled ? '18px' : '2px'}}></div>
                </div>
            </button>
        </div>
        ...
        <div className="space-y-6 mb-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            {aiEnabled && (
                <div className="bg-purple-50 p-5 rounded-2xl border border-purple-100 shadow-sm animate-fadeIn">
